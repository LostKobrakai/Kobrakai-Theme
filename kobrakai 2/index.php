	...
	<div id="container">
		<header>
			<hgroup>
				...
				<?php wp_nav_menu(array('menu' => 'main_menu', 'before' => '&rsaquo; ', 'depth' => 0)); ?>
			</nav><!-- /#nav  -->
		</header><!-- /header -->		
		<section id="main">
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?><article class="list">
            <div class="media">
                <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" rel="bookmark"><?php the_post_thumbnail('full'); ?></a>
                <div class="info">
                    <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" rel="bookmark"><h3><?php the_title(); ?></h3></a> &sdot; <?php the_time('d F, Y'); ?>
                    <p><?php the_content(); ?></p>
                </div>
            </div>
